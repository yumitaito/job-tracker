...
import { assertSafeTarget, type DnsLookupResult, isBlockedHost } from "./security.ts";
...
function extractSalary(jobPosting: any): { min?: number; max?: number } {
  ...
  const toManYen = (
    n?: number,
  ) => (n === undefined ? undefined : Math.round((n * multiplier) / 10000));
...
export type FetchHtmlDependencies = {
  fetchImpl?: typeof fetch;
  assertTarget?: (target: URL) => Promise<void>;
  timeoutMs?: number;
};

export async function fetchHtml(url: string, deps: FetchHtmlDependencies = {}): Promise<string> {
  ...
  try {
    ...
    for (let redirects = 0; redirects <= 5; redirects += 1) {
      if (deps.assertTarget) await deps.assertTarget(target);
      else {
        const allowedHosts = (Deno.env.get("JOB_METADATA_ALLOWED_HOSTS") ?? "")
          .split(",").map((host) => host.trim().toLowerCase()).filter(Boolean);
        await assertSafeTarget(target, allowedHosts, resolveDnsRecords);
      }
...
function buildMetadata(
  html: string,
): { source: "structured" | "meta" | "none"; data: JobMetadata } {
...
export function createFetchJobMetadataHandler(
  ...
  return async (req: Request): Promise<Response> => {
    ...
    try {
      const html = await (deps.fetchHtml ?? fetchHtml)(target.toString());
